...
int _set_log_file_name(struct nlogger_log_struct *log, const char *log_file_name) {
    ...
    if (log->p_name != NULL) {
        memset(log->p_name, 0, file_name_size);
    ...
    LOGD("check", "_config_log_file_name finish log file name >>> %s, log file path >>> %s.", log->p_name, log->p_path);
    return ERROR_CODE_OK;
}
 ...
 */
int _check_log_file(struct nlogger_log_struct *log, const char *log_file_name) {
    LOGD("check", "#### start check log file !!!");
//    这一步放在外面来判断，也就是write的地方
//    if (g_nlogger == NULL || g_nlogger->state == NLOGGER_STATE_ERROR) {
//        return ERROR_CODE_NEED_INIT_BEFORE_ACTION;
//    }
...
//        memcpy(log->p_name, log_file_name, file_name_size);
        ...
        if (!is_empty_string(log->p_path) &&
            ...
            _check_log_file(log, log_file_name);
        }
    }
    ...
    if (log->state == NLOGGER_LOG_STATE_CLOSE) {
        ...
        if (result == ERROR_CODE_OK) {
            //表示第一次打开日志文件
            return ERROR_CODE_FIRST_OPEN_LOG_FILE;
        }
        //打开文件失败的情况
        return result;
    }
...
int open_log_file(const char *log_file_name) {
    _check_log_file(&g_nlogger->log, log_file_name);
    _map_log_name_to_mmap_cache(&g_nlogger->cache, log_file_name);
    return 1;
}
...
int write_nlogger(const char *log_file_name, int flag, char *log_content, long long local_time, char *thread_name, long long thread_id, int is_main) {
    ...
    int result = _check_log_file(&g_nlogger->log, log_file_name);

    //第一次创建或者打开日志文件，需要重新指定当前mmap缓存指向的日志文件，方便下次启动将缓存flush到指定的日志文件中
    if (result == ERROR_CODE_FIRST_OPEN_LOG_FILE) {
        if (g_nlogger->cache.cache_mode == NLOGGER_MMAP_CACHE_MODE) {
            //会写入头，并且指定
            int map_result = _map_log_name_to_mmap_cache(&g_nlogger->cache, log_file_name);
            //建立日志与mmap缓存的映射关系失败，则切换成Memory缓存模式
            //（不能建立关系就失去了意外中断导致日志丢失的优势，这和内存缓存没有区别）
            if (map_result != ERROR_CODE_OK) {
                if (g_nlogger->cache.p_buffer != NULL) {
                    munmap(g_nlogger->cache.p_buffer, NLOGGER_MMAP_CACHE_SIZE);
                }
                //路径传空，强制Memory模式缓存
                if (NLOGGER_INIT_CACHE_FAILED == init_cache_nlogger(NULL, &g_nlogger->cache.p_buffer)) {
                    return NLOGGER_INIT_CACHE_FAILED;
                }
            }
        }
        //mmap模式会在完成映射关系以后指定，代表content长度指针的为止
        //memory模式因为不需要进行文件关系绑定，因此需要手动指定，也就是内存的开头
        //这里不能直接else  或者 else if 因为上面这块可能会切换mode缓存形式
        if (g_nlogger->cache.cache_mode == NLOGGER_MEMORY_CACHE_MODE) {
            g_nlogger->cache.p_length = g_nlogger->cache.p_buffer;
        }

        g_nlogger->cache.p_next_write   = g_nlogger->cache.p_length + NLOGGER_CONTENT_LENGTH_BYTE_SIZE;
        g_nlogger->cache.length         = 0;
        g_nlogger->cache.content_length = 0;
    }

    //step2 检查缓存
    //mmap缓存文件不见了，这个时候主动切换成Memory缓存
    if (g_nlogger->cache.cache_mode == NLOGGER_MMAP_CACHE_MODE && !is_file_exist_nlogger(g_nlogger->cache.p_file_path)) {
        if (g_nlogger->cache.p_buffer != NULL) {
            munmap(g_nlogger->cache.p_buffer, NLOGGER_MMAP_CACHE_SIZE);
        }
        //路径传空，强制Memory缓存
    ...
    if (g_nlogger->cache.cache_mode == NLOGGER_MEMORY_CACHE_MODE && g_nlogger->cache.p_buffer == NULL) {
        return NLOGGER_INIT_CACHE_FAILED;
    }
